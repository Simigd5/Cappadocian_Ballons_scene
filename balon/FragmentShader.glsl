...
uniform vec3 light_position1;  // Poziția sursei de lumină punctiformă
uniform vec3 light_color1;     // Culoarea luminii punctiforme
uniform vec3 light_position2;  // Poziția sursei de lumină punctiformă
uniform vec3 light_color2;     // Culoarea luminii punctiforme
uniform vec3 light_position3;  // Poziția sursei de lumină punctiformă
uniform vec3 light_color3;     // Culoarea luminii punctiforme
uniform vec3 light_position4;  // Poziția sursei de lumină punctiformă
uniform vec3 light_color4;     // Culoarea luminii punctiforme
uniform vec3 light_position5;  // Poziția sursei de lumină punctiformă
uniform vec3 light_color5;     // Culoarea luminii punctiforme
uniform vec3 light_position6;  // Poziția sursei de lumină punctiformă
uniform vec3 light_color6;     // Culoarea luminii punctiforme
uniform vec3 light_position7;  // Poziția sursei de lumină punctiformă
uniform vec3 light_color7;     // Culoarea luminii punctiforme
uniform vec3 light_position8;  // Poziția sursei de lumină punctiformă
uniform vec3 light_color8;     // Culoarea luminii punctiforme
uniform vec3 light_position9;  // Poziția sursei de lumină punctiformă
uniform vec3 light_color9;     // Culoarea luminii punctiforme
uniform vec3 light_direction; // Direcția luminii direcționale
uniform vec3 dir_light_color; // Culoarea luminii direcționale
...
void main()
{
    ...
    // --- Point Light ---
    //1
    vec3 L_point = normalize(light_position1 - frag_world_position);
    float distance_to_light = length(-light_position1 + frag_world_position);
    float attenuation = 1.0 / (1.0 + 0.14 * distance_to_light + 0.07 * distance_to_light * distance_to_light);

    float diffuse_point = max(dot(N, L_point), 0.0);
    float specular_point = 0.0;

    if (diffuse_point > 0.0) {
        vec3 H_point = normalize(L_point + V);
        specular_point = pow(max(dot(N, H_point), 0.0), material_shininess);
    }

    vec3 point_light_color = attenuation*1.8 * light_color1 * 
                             (material_kd * diffuse_point + material_ks * specular_point*0.5);
    ...
    vec3 point_light_color2 = attenuation2*1.8 * light_color2 * 
                             (material_kd * diffuse_point2 + material_ks * specular_point2*0.5);
...
//3
    ...
    if (diffuse_point3 > 0.0) {
        vec3 H_point3 = normalize(L_point2 + V);
        specular_point3 = pow(max(dot(N, H_point3), 0.0), material_shininess);
    }

    vec3 point_light_color3 = attenuation3*1.8 * light_color3 * 
                             (material_kd * diffuse_point3 + material_ks * specular_point3*0.5);
...
//4
    ...
    vec3 point_light_color4 = attenuation4*1.8 * light_color4 * 
                             (material_kd * diffuse_point4 + material_ks * specular_point4*0.5);
    ...
    vec3 point_light_color5 = attenuation5*1.8 * light_color5 * 
                             (material_kd * diffuse_point5 + material_ks * specular_point5*0.5);
...
//6
    ...
    vec3 point_light_color6 = attenuation6*1.8 * light_color6 * 
                             (material_kd * diffuse_point6 + material_ks * specular_point6*0.5);
...
//7
    ...
    vec3 point_light_color7 = attenuation7*1.8 * light_color7 * 
                             (material_kd * diffuse_point7 + material_ks * specular_point7*0.5);
    //8
    ...
    vec3 point_light_color8 = attenuation8*1.8 * light_color8 * 
                             (material_kd * diffuse_point8 + material_ks * specular_point8*0.5);
    ...
    float attenuation9 = 1.0 / (1.0 + 0.14 * distance_to_light8 + 0.07 * distance_to_light9 * distance_to_light9);
    ...
    vec3 point_light_color9 = attenuation9*1.8 * light_color9 * 
                             (material_kd * diffuse_point9 + material_ks * specular_point9*0.5);
    // --- Directional Light ---
    vec3 L_dir = normalize(-light_direction);
    float diffuse_dir = max(dot(N, L_dir), 0.0);
    float specular_dir = 0.043;
    ...
    vec3 dir_light_color_final = dir_light_color * 1.5*(material_kd * diffuse_dir*2.1 + material_ks * specular_dir*0.25);
    
    // --- Ambient Light ---
    float ambient_strength = 0.1;
    vec3 ambient_color = ambient_strength * object_color;

    // --- Combine Lighting ---
    vec3 final_light_color = ambient_color + point_light_color+point_light_color2+point_light_color3+point_light_color4+point_light_color5+point_light_color6+point_light_color7+point_light_color8+point_light_color9 + dir_light_color_final;
